...
@section('conten')
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            ...
            <div class="icon mb-3">
                <a href="/rate-contract/astim/oudistance-lcm/tambah" type="button" class="btn btn-success"><i></i>Tambah</a>
            </div>

            <!-- Form untuk pencarian -->
            <div class="mb-4">
                <form method="GET" action="{{ url('/rate-contract/astim/oudistance-lcm') }}"
                    ...
                    <tbody>
                        ...
                        @foreach ($dokument as $key => $item)
                            <tr>
                                <td>{{ $No++ }}</td>
                                <td>
                                <a class="btn" href="{{ url('rate-contract/astim/oudistance-lcm/view/' . $item->id_contract) }}"> {{ $item->created_at }}</a>
                                </td>
                                <td>{{ $item->name_contract}}</td>
                                <td>
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="{{ url('rate-contract/astim/oudistance-lcm/detail/' . $item->id_contract) }}">
                                            <i class="ri-information-line" title="Detail"></i>
                                        </a>
                                        <a href="{{ url('rate-contract/astim/oudistance-lcm/edit/' . $item->id_contract) }}">
                                            <i class="ri-edit-2-line text-warning" title="Edit"></i>
                                        </a>
                                        <form action="{{ url('/rate-contract/astim/oudistance-lcm/delete/' . $item->id_contract) }}"
                                            method="POST"
